...
def home(request):
    rooms = Register.objects.all()
    context = {'rooms' : rooms}
    return render(request, 'Usregandlog/home.html', context)
...
def createroom(request):
    ...
    context={'form' : form}
    return render(request, 'Usregandlog/room_form.html', context)

def checking_iris(request):
    ...
    for file in [file for file in os.listdir("final")]:
        ...
        for p, q in matches:
            ...
            if (len(match_points) / keypoints) > 0.95:
                ...
                result = cv2.drawMatches(test_original, keypoints_1, iris_database_image,keypoints_2, match_points, None)
